...
function Order({dish}) {
    ...
    const quitarCantidad=()=>{
        ...
        else{setCantidad(cantidad-1); setMontoTotal((cantidad-1)*precio) }
...
<div className="text-xl w-full text-center font-bold text-gray-700 mb-4">
       ...
       <div className="flex flex-row justify-center items-center w-full">
        ...
        <div className="text-3xl text-center mx-2 bg-gray-100 w-1/4 text-gray-800 rounded-md">{cantidad}</div>

        <button className="text-4xl bg-yellow-500 text-gray-800 rounded-full" onClick={() => quitarCantidad()}>
          <MdRemove/>
          </button>
          </div>

<h1 className="text-lg text-center w-fulltext-gray-700">Total: ${montoTotal.toLocaleString('de-DE')}</h1>
